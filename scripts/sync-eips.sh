...
# 6. Commits and pushes if there are changes
#
# Usage: bash scripts/sync-eips.sh
# Designed to run in a GitHub Actions workflow (ubuntu-latest).
...
if [ "$CURRENT_BRANCH" != "$TARGET_BRANCH" ]; then
    ...
    git checkout "$TARGET_BRANCH" 2>/dev/null || {
        echo "ERROR: Could not switch to $TARGET_BRANCH branch."
        exit 1
    }
fi
...
CHANGED_FILES=$(git diff --name-only "upstream/$UPSTREAM_BRANCH" -- "$EIPS_DIR/" || true)
NEW_COUNT=0
MODIFIED_COUNT=0
SKIPPED_COUNT=0
ERC_UPDATES=0
...
if [ -z "$CHANGED_FILES" ]; then
    ...
    while IFS= read -r file; do
        ...
        if [ ! -f "$file" ]; then
            ...
            # Modified file - checkout from upstream (except ERCs, handled separately)
            echo "  [~] MODIFIED: $file"
            git checkout "upstream/$UPSTREAM_BRANCH" -- "$file" 2>/dev/null || {
                echo "  [!] SKIP: Could not checkout $file"
                continue
...
if [ -f "$SCRIPT_DIR/fetch-erc-metadata.py" ]; then
    ...
    ERC_RESULT=$?
    if [ $ERC_RESULT -eq 0 ]; then
        echo "✓ ERC metadata updated successfully."
    else
        echo "⚠ WARNING: ERC metadata script exited with code $ERC_RESULT"
    fi
else
